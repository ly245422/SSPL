    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            if self.trainset == 'ChaoticWorld':
                data_path = self.args.data_path + 'ChaoticWorld/all_unlabeled/'

            # frame
            with h5py.File(data_path + 'h5py_train_frames.h5', 'r') as hf:
                frame = hf['train_frames'][index, :, :, :]   # (256, 256, 3)
                frame_list = self._load_frame(frame=frame)
            # audio
            with h5py.File(data_path + 'h5py_train_audios.h5', 'r') as hf:
                audio = hf['train_audios'][index, :]   # (48000,)
            # log-mel spectrogram
            with h5py.File(data_path + 'h5py_train_spects.h5', 'r') as hf:
            ...
            elif self.testset == 'ChaoticWorld':
                data_path = self.args.data_path + 'ChaoticWorld/labeled/Data/'
                # frame
                with h5py.File(data_path + 'h5py_test_frames.h5', 'r') as hf:
                    frame = hf['test_frames'][index, :, :, :]   # (256, 256, 3)
                    frame_list = self._load_frame(frame=frame)
